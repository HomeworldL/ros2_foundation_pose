...
class FoundationPoseNode(Node):
    ...
    def image_callback(self, depth_msg: Image, color_msg: Image):
        tstart = time.time()
        ...
        # self.get_logger().info(f"depth_timestamp: {depth_timestamp}")
        # self.get_logger().info(f"color_timestamp: {color_timestamp}")
        ...
        # self.get_logger().info(f"pose x: {pose[0][3]}")
        # self.get_logger().info(f"pose x: {pose[0,3]}")
        pose.astype('float')
        
        center_pose = pose@np.linalg.inv(self.to_origin)
        vis = draw_posed_3d_box(self.K, img=color_image, ob_in_cam=center_pose, bbox=self.bbox)
        vis = draw_xyz_axis(color_image, ob_in_cam=pose, scale=0.1, K=self.K, thickness=3, transparency=0, is_input_rgb=True)
        cv2.imshow('1', vis[...,::-1])
        cv2.waitKey(1)
